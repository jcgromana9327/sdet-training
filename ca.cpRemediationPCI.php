...
class MySeleniumSuite extends PHPUnit_Extensions_Selenium2TestCase {
    ...
    public function setUp(){
        ...
        $this -> setBrowserUrl($this -> configEnvironment["ca.cp-production"]);
        ...
        $this -> filename = __DIR__ . "/report/ca.CPPciRemediation-result.html";

        $this -> fp = fopen($this -> filename, 'w');
        $data = 'Environment: ' . $this -> configEnvironment["ca.cp-production"] .$this -> templateHeader["sql-header"];
        fwrite($this->fp, $data);
    }
...
public function additem(){
  $url = $this -> configPageUrl["ca.cp-additem"];
  $inject = $this -> configInjection["ca.cp-code-injection"];
  $inject1 = $this -> configInjection["ca.cp-code-injection-1"];
  $sqlinject = $this -> configInjection["ca.cp-sql-injection"];

  $fields = array(
  "qty" =>  rawurlencode("1"),
  "year" =>  rawurlencode("2010"),
  "make" =>  rawurlencode($inject1),
  "model" =>  rawurlencode("MDX"),
  "submodel" =>  rawurlencode("Base"),
  "engine" =>  rawurlencode($inject),
  ...
  // print_r($fields);
  echo "\n\nValidate Additem Code Injection Vulnerability - CA.CP Remediation";
  $formName = "Code Injection";
  ...
  $content = $this->loadCurlHeader($url ,$fields , "-");
  // print_r($fields);
  echo "\n\nValidate Additem SQL Injection Vulnerability - CA.CP Remediation";
  $formName = "SQL Injection";
  $this->validateStatusCode($url,$content,$formName);
