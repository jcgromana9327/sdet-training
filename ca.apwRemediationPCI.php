...
class MySeleniumSuite extends PHPUnit_Extensions_Selenium2TestCase {
    ...
    public function setUp(){
        ...
        $this -> setBrowserUrl($this -> configEnvironment["ca.apw-staging"]);
        ...
        $this -> filename = __DIR__ . "/report/ca.APWPciRemediation-result.html";

        $this -> fp = fopen($this -> filename, 'w');
        $data = 'Environment: ' . $this -> configEnvironment["ca.apw-staging"] .$this -> templateHeader["sql-header"];
        fwrite($this->fp, $data);
    }
...
public function additem(){
  $url = $this -> configPageUrl["ca.apw-additem"];
  $inject = $this -> configInjection["ca.apw-code-injection"];
  $sqlinject = $this -> configInjection["ca.apw-sql-injection"];

  $fields = array(
  "qty" =>  rawurlencode("1"),
  "year" =>  rawurlencode("2010"),
  "make" =>  rawurlencode($inject),
  ...
  // print_r($fields);
  echo "\n\nValidate Additem Code Injection Vulnerability - CA.APW Remediation";
  ...
  $content = $this->loadCurlHeader($url ,$fields , "-");
  // print_r($fields);
  echo "\n\nValidate Additem SQL Injection Vulnerability - CA.APW Remediation";
